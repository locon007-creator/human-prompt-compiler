...
describe('Drop & Hook Assistant complexity benchmark', () => {
  it('preserves linear workflow, app structure, route editing, trailer continuity, persistence, boundaries, and Arena-safe delivery', () => {
    ...
    expect(prompt).toMatch(/add(?:,|\/)\s*edit(?:,|\/)\s*remove(?:,|\/)\s*(?:and\s*)?reorder route stops/i)
    expect(prompt).toMatch(/Route & Equipment/i)
    expect(prompt).toMatch(/half-height bottom sheet/i)
    expect(prompt).toMatch(/Attach collapsible Drop & Hook/i)
    ...
    expect(prompt).toMatch(/closing\/reloading/i)
    ...
    expect(prompt).toMatch(/No React, Vite, npm, JSX/i)
    expect(prompt).toMatch(/No simulated device chrome/i)
